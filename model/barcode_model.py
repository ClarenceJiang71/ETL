...
# 第一步：导入模块
from util.str_util import clean_str
from util.str_util import check_str_null_and_transform_to_sql_null, check_number_null_and_transform_to_sql_null
...
class BarcodeModel(object):
    ...
    def __init__(self, data_tuple:tuple):  # 数据是以元组方式发送给data_tuple => ('014779000888', '日本眉机', '', '', '', '个', 'null', 0.0000, 0.0000, '2017-07-10 16:56:44', '', 0, 'null', 'null')
        ...
        self.name = clean_str(data_tuple[1])
        self.spec = clean_str(data_tuple[2])
        self.trademark = clean_str(data_tuple[3])
        self.addr = clean_str(data_tuple[4])
        self.units = clean_str(data_tuple[5])
        self.factory_name = clean_str(data_tuple[6])
        self.trade_price = data_tuple[7]
        self.retail_price = data_tuple[8]
        self.update_at = str(data_tuple[9])
        self.wholeunit = clean_str(data_tuple[10])
